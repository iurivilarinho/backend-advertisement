...
public class DisplaySpecification {
	public static Specification<Display> customerIdEquals(Long customerId) {
		if (customerId == null) {
	...
	public static Specification<Display> activeEquals(Boolean active) {
		if (active == null) {
			return Specification.unrestricted();
		}
		return (root, query, builder) -> builder.equal(root.get("active").get("id"), active);
	}
	
	public static Specification<Display> searchAllFields(String searchTerm, EntityManager entityManager) {
